...
public class DefaultMovieService : IMovieService
{
	...
	public async Task<Result<Movie>> UpdateMovie(int id, Movie newData)
	{
		...
		var result = movie == null
			? new Result<Movie>(
				new Exception($"Could not update movie {newData} with id {id}."), (int)HttpStatusCode.NotFound)
			: new Result<Movie>(movie, (int)HttpStatusCode.OK);
		return result;
	}
	...
	private static Result<Movie>? ValidateMovie(Movie? movieData)
	{
		if (movieData is null)
		{
			...
			new Exception("Movie payload is required."), (int)HttpStatusCode.BadRequest);
		}
		if (string.IsNullOrWhiteSpace(movieData.Title))
		{
			return new Result<Movie>(
			new Exception("Title is required and cannot be empty."), (int)HttpStatusCode.BadRequest);
		}
		if (movieData.Title.Length > 256)
		{
			return new Result<Movie>(
			new Exception("Title cannot be longer than 256 characters."), (int)HttpStatusCode.BadRequest);
		}
		if (movieData.Year < 1888 || movieData.Year > DateTime.UtcNow.Year)
		{
			return new Result<Movie>(
			new Exception($"Year must be between 1888 and {DateTime.UtcNow.Year}."), (int)HttpStatusCode.BadRequest);
		}
		return null;
	}
}